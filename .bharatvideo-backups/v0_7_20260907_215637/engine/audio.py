...
def _generate_with_bharatvideo_neural_tts(text: str, dest: Path, *, character_id: str, sample_rate: int) -> str:
    template = os.getenv("CONTENT_FACTORY_TTS_CMD", "").strip()
    if not template:
        return ""
    with tempfile.TemporaryDirectory(prefix="acf-neural-tts-") as td:
        text_file = Path(td) / "speech.txt"
        raw_out = Path(td) / "speech.wav"
        text_file.write_text(text, encoding="utf-8")
        command = template.format(text_file=str(text_file), output=str(raw_out), character=character_id, reference_wav="")
        proc = subprocess.run(shlex.split(command), stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
        if proc.returncode or not raw_out.exists():
            raise AudioGenerationError("BharatVideo neural TTS failed: " + (proc.stderr[-2000:] or command))
        _normalize_wav(raw_out, dest, sample_rate=sample_rate)
    return "bharatvideo_neural_tts"
...
def generate_scene_audio(
    ...
    *,
    ...
    for turn in scene.dialogue:
        ...
        used = _generate_with_bharatvideo_neural_tts(
            spoken_text, raw_path, character_id=turn.character_id, sample_rate=sample_rate,
        )
        if not used and kanpuri_voice:
    ...
    # V6.6 measures the *actual* generated TTS before deciding whether turns fit.
    # App-created projects often start with tiny generic scene slots (e.g. 3s).
    # Growing the transient render budget here prevents the legacy turn selector
    # from dropping valid dialogue merely because the authoring estimate was low.
    full_speech_total = sum(float(x["raw_duration"]) for x in raw_segments)
    full_pause_total = sum(pauses_all)
    pre_extension_original_budget = budget
    pre_extension_required_budget = (
        (full_speech_total / max_speedup) + full_pause_total + safety + 0.18
    )
    pre_extended = pre_extension_required_budget > budget + 0.01
    if pre_extended:
        budget = pre_extension_required_budget
        print(
            f"[VOICE PLAN AUTO-EXTEND V6.6] scene={scene.id} "
            f"budget={pre_extension_original_budget:.2f}s->{budget:.2f}s "
            f"full_raw_speech={full_speech_total:.2f}s turns={len(raw_segments)} "
            "policy=PRESERVE_COMPLETE_TURNS"
        )
    ...
    if pre_extended:
        dialogue_budget["mode"] = "natural_duration_pre_extend"
        dialogue_budget["original_budget_seconds"] = round(pre_extension_original_budget, 4)
        dialogue_budget["extended_budget_seconds"] = round(budget, 4)
        dialogue_budget["auto_extended_for_natural_voice"] = True
        dialogue_budget["complete_turns_preserved"] = (len(selected_indices) == len(raw_segments))
    ...
    if speedup > max_speedup + 0.003:
        # BharatVideo V0.5 / renderer V6.6: never force natural dialogue into an
        # undersized app-authored scene slot.  At this point the *actual* TTS
        # waveforms are known, so grow the transient render budget instead of
        # exceeding the natural voice cap or failing the entire render.
        original_budget = budget
        minimum_natural_budget = (speech_total / max_speedup) + pause_total + safety + 0.18
        budget = max(budget, minimum_natural_budget)
        available_speech = max(0.5, budget - pause_total - safety)
        speedup = max(1.0, speech_total / available_speech)
        dialogue_budget["mode"] = "natural_duration_auto_extend"
        dialogue_budget["original_budget_seconds"] = round(original_budget, 4)
        dialogue_budget["extended_budget_seconds"] = round(budget, 4)
        dialogue_budget["auto_extended_for_natural_voice"] = True
        print(
            f"[VOICE BUDGET AUTO-EXTEND V6.6] scene={scene.id} "
            f"budget={original_budget:.2f}s->{budget:.2f}s "
            f"raw={raw_total:.2f}s required_speedup={speedup:.3f}x "
            f"cap={max_speedup:.2f}x"
        )

    if speedup > max_speedup + 0.003:
        raise AudioGenerationError(
            f"[VOICE NATURALNESS QA] scene={scene.id} "
            f"selected_raw={raw_total:.2f}s auto_extended_budget={budget:.2f}s "
            f"required_speedup={speedup:.3f}x > natural_cap={max_speedup:.2f}x. "
            "Automatic natural-duration extension could not reconcile this scene."
        )
